...
const siteData = deserializeFunctions(JSON.parse('{"lang":"ru-RU","dir":"ltr","title":"Fullstack Knowledge Graph","description":"База знаний и граф по frontend и backend технологиям","base":"/","head":[],"router":{"prefetchLinks":true},"appearance":true,"themeConfig":{"logo":"/logo.svg","nav":[{"text":"Граф","link":"/graph"},{"text":"Frontend","link":"/frontend/foundation/"},{"text":"Backend","link":"/backend/"},{"text":"Meta","link":"/meta/add-node"}],"sidebar":{"/frontend/":[{"text":"Frontend","items":[{"text":"Foundation","link":"/frontend/foundation/"},{"text":"UI","link":"/frontend/ui/"},{"text":"State","link":"/frontend/state/"},{"text":"Forms & Validation","link":"/frontend/forms-validation/"},{"text":"Data","link":"/frontend/data/"},{"text":"Utils","link":"/frontend/utils/"},{"text":"Testing","link":"/frontend/testing/"},{"text":"Tooling","link":"/frontend/tooling/"}]}],"/backend/":[{"text":"Backend","items":[{"text":"Overview","link":"/backend/"},{"text":"Foundation","link":"/backend/foundation/"},{"text":"API","link":"/backend/api/"},{"text":"Data","link":"/backend/data/"},{"text":"Infrastructure","link":"/backend/infrastructure/"},{"text":"Testing","link":"/backend/testing/"}]}],"/meta/":[{"text":"Meta","items":[{"text":"Как добавлять узлы","link":"/meta/add-node"}]}]},"search":{"provider":"local"},"socialLinks":[{"icon":"github","link":"https://github.com/AndrewGanzha/fullstack-graph"}]},"locales":{},"scrollOffset":134,"cleanUrls":true}'));
...
const _sfc_main$t = /* @__PURE__ */ defineComponent({
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const VPLocalSearchBox = defineAsyncComponent(() => import("./VPLocalSearchBox.B-djZvPP.js"));
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const nodesData = yaml.load(nodesRaw) || [];
    const edgesData = yaml.load(edgesRaw) || [];
    const query = ref("");
    const area = ref("all");
    ...
    const applyFilters = () => {
      ...
      const visibleIds = new Set(
        nodesData.filter((node) => areaFilter === "all" ? true : node.area === areaFilter).filter((node) => matchesQuery(node, q)).map((node) => node.id)
      );
      ...
      cy.edges().forEach((edge) => {
        const visible = visibleIds.has(edge.source().id()) && visibleIds.has(edge.target().id());
        edge.style("display", visible ? "element" : "none");
      });
      cy.nodes().removeClass("matched");
      if (q) {
        cy.nodes().filter((node) => visibleIds.has(node.id())).filter((node) => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(mergeProps({ class: "graph-wrapper" }, _attrs))} data-v-b53e4096><header class="graph-header" data-v-b53e4096><div data-v-b53e4096><p class="graph-kicker" data-v-b53e4096>Fullstack Knowledge Graph</p><h2 data-v-b53e4096>Interactive map</h2><p class="graph-subtitle" data-v-b53e4096> Nodes: ${ssrInterpolate(totalNodes.value)} · Edges: ${ssrInterpolate(totalEdges.value)}</p></div><div class="graph-actions" data-v-b53e4096><button class="ghost" type="button" data-v-b53e4096>Reset zoom</button><button class="solid" type="button" data-v-b53e4096>Rebuild</button></div></header><div class="graph-controls" data-v-b53e4096><label data-v-b53e4096><span data-v-b53e4096>Search</span><input${ssrRenderAttr("value", query.value)} type="search" placeholder="Vue, Laravel, FastAPI, PostgreSQL..." data-v-b53e4096></label><label data-v-b53e4096><span data-v-b53e4096>Area</span><select data-v-b53e4096><!--[-->`);
      ssrRenderList(areas.value, (item) => {
        _push(`<option${ssrRenderAttr("value", item)} data-v-b53e4096${ssrIncludeBooleanAttr(Array.isArray(area.value) ? ssrLooseContain(area.value, item) : ssrLooseEqual(area.value, item)) ? " selected" : ""}>${ssrInterpolate(item === "all" ? "All" : item)}</option>`);
      });
      _push(`<!--]--></select></label><label class="toggle" data-v-b53e4096><input${ssrIncludeBooleanAttr(Array.isArray(showLabels.value) ? ssrLooseContain(showLabels.value, null) : showLabels.value) ? " checked" : ""} type="checkbox" data-v-b53e4096><span data-v-b53e4096>Show labels</span></label></div><div class="graph-body" data-v-b53e4096><div class="graph-canvas" data-v-b53e4096></div><aside class="graph-legend" data-v-b53e4096><h3 data-v-b53e4096>Edge legend</h3><ul data-v-b53e4096><!--[-->`);
      ssrRenderList(EDGE_TYPES, (edge) => {
        _push(`<li data-v-b53e4096><span class="swatch" style="${ssrRenderStyle({ background: edge.color })}" data-v-b53e4096></span><span class="label" data-v-b53e4096>${ssrInterpolate(edge.label)}</span></li>`);
      });
      _push(`<!--]--></ul></aside></div></section>`);
    };
  }
});
const _sfc_setup = _sfc_main.setup;
...
const GraphView = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-b53e4096"]]);
const RawTheme = {
  ...theme,
  enhanceApp({ app }) {
